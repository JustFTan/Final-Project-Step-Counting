...
from gui_common import * # Make sure cachedData is defined/imported here if it's in gui_common
from cachedData import * # This is where cachedDataType is imported
# from fall_detection import FallDetection, fallDetectionSliderClass
from buzzer import *
from threading import Lock

# --- Start of new/reordered definitions ---
base_dir = "C:/Users/justf/Desktop/Telkom/Sem 6/MBKM Penelitian/Radar/Tugas-Akhir-FYS1-main/Tugas-Akhir-FYS1-main"

# CachedData holds the data from the last configuration run for faster prototyping and testing
# Initialize cachedData *HERE* so it's guaranteed to exist before Window.__init__ is called.
...
# --- Defines for Deep Learning Model ---


from collections import deque
import time
# Variabel global tambahan
last_pred_time = 0  # Waktu terakhir prediksi

step_batch = deque(maxlen=23)
window_ready = False  # Tambahkan flag



# Initializing `df` with zeros (used by older ML model approach or other GUI parts)
# Keep if still needed, otherwise consider removing.
df = [
    [0] * len(columns), # Using len(columns) for consistency
    [0] * len(columns),
    [0] * len(columns),
    [0] * len(columns),
    [0] * len(columns),
...
sendJatuh = True
sendFatal = True
model = joblib.load(os.path.join("C:/Users/justf/Desktop/Telkom/Sem 6/MBKM Penelitian/Radar/Tugas-Akhir-FYS1-main/Tugas-Akhir-FYS1-main/Model/knn_multitask_steps_dist_best.pkl"))
# model = joblib.load(os.path.join("C:/Users/justf/Desktop/Telkom/Sem 6/MBKM Penelitian/Radar/Tugas-Akhir-FYS1-main/Tugas-Akhir-FYS1-main/Model/et_multitask_steps_dist.pkl"))
# model = joblib.load(os.path.join("C:/Users/justf/Desktop/Telkom/Sem 6/MBKM Penelitian/Radar/Tugas-Akhir-FYS1-main/Tugas-Akhir-FYS1-main/Model/rf_multitask_steps_dist_new.pkl"))
# Global variable for the Keras model, loaded once
loaded_keras_model = None

# Preprocess Data
def step_distance_est(X_flat):
    global step_pred, dist_pred
...
def get_trackColors(n):
    # ... (rest of your get_trackColors function)
    modKellyColors = [
    ...
    for i in range(n):
        ...
        if i < len(modKellyColorsNorm):
            trackColorList.append(modKellyColorsNorm[i]) # Changed index from 0 to i
...
def visualizePointCloud(heights, tracks, self):
    ...
    global df, step_temp, dist_temp, last_pred_time, step_batch
        ...
        for height in heights:
            for track in tracks:
                if int(track[0]) == int(height[0]):
                    ...
                    yvel = track[5]
                    zvel = track[6]
                    vmag = np.sqrt(xvel ** 2 + yvel ** 2 + zvel ** 2)
                    
                    height_val = 1.73  # fallback default

                    step_batch.append([xvel, yvel, zvel, vmag, height_val])

                    # 2. Jika sudah 23 frame DAN minimal 1 detik sejak prediksi terakhir
                    if len(step_batch) == 23 and (time.time() - last_pred_time) >= 1.0:
                        MIN_MOVEMENT = 0.4  # batas minimum pergerakan

                        # Hitung rata-rata vmag
                        vmag_values = [row[3] for row in step_batch]
                        avg_vmag = sum(vmag_values) / len(vmag_values)

                        if avg_vmag < MIN_MOVEMENT:
                            print(f"[INFO] Skipping prediction (low movement: avg_vmag={avg_vmag:.2f})")
                            step_batch.clear()
                            return

                        # Buat DataFrame untuk model
                        df = pd.DataFrame(step_batch)

                        # Validasi shape
                        if df.shape != (23, 5):
                            print(f"[WARNING] Invalid shape: {df.shape}, expected (23, 5)")
                            step_batch.clear()
                            return

                        # Prediksi
                        step_pred, dist_pred = predictModel(df)

                        # Filter prediksi langkah
                        if step_pred < 0.8:
                            rounded_step = 0
                        elif  step_pred < 1.6:
                            rounded_step = math.floor(step_pred)
                        else:
                            rounded_step = math.ceil(step_pred)

                        if rounded_step >= 1:
                            step_temp += rounded_step
                            dist_temp += dist_pred
                        else:
                            print(f"[INFO] Rounded step {rounded_step} too small, ignored.")

                        # Update waktu terakhir prediksi
                        last_pred_time = time.time()

                        # Geser window (hapus semua frame, mulai kumpul batch baru)
                        step_batch.clear()

                    # 3. Update GUI
                    self.positionLabel.setText(f"Posisi: ({xPos:.2f}, {yPos:.2f}, {zPos:.2f})")
                    self.statusLabel.setText("Status: Jalan")
                    self.statusLabel.setStyleSheet("color: green; font-size: 16px; font-weight: bold;")

                    # 4. Update langkah & jarak
                    self.stepsLabel.setText(f"Steps: {round(step_temp)}")
                    self.distanceLabel.setText(f"Distance: {dist_temp:.2f} m")

    else:
        self.statusLabel.setText("Status: Diam")


# def getHeightFromPointCloud(pointCloud):
#     """
#     Ambil tinggi rata-rata dari point cloud per frame.
#     Tinggi dihitung dari rata-rata nilai z semua titik.
#     """
#     if not pointCloud or len(pointCloud) == 0:
#         return None

#     # Ambil semua z dari point cloud
#     z_values = [p[2] for p in pointCloud]

#     # Hitung rata-rata z
#     avg_height = sum(z_values) / len(z_values)
#     return avg_height



# Taruh di awal file (jika belum ada)

def predictModel(df):
    try:
        if df.shape != (23, 5):
            print(f"[WARNING] Unexpected shape: {df.shape}, skipping prediction.")
            return 0, 0

        X = df.values  # tanpa scaler
        preds = model.predict(X.reshape(1, -1))  # misalnya RandomForest multitask
        step_pred, dist_pred = preds[0]
        print(f"[MODEL OUTPUT] Steps: {step_pred}, Distance: {dist_pred:.2f}")
        return step_pred, dist_pred

    except Exception as e:
        print(f"[ERROR in predictModel] {e}")
        return 0, 0
...
class Window(QDialog):
    ...
    def fallCondition(self):
        ...
        # Tambahkan label untuk status klasifikasi
        self.statusLabel = QLabel("Status: Diam")
        self.statusLabel.setStyleSheet("font-size: 16px; font-weight: bold;")

        self.positionLabel = QLabel("Posisi: (0, 0, 0)")
        self.positionLabel.setStyleSheet("font-size: 14px;")
        
        # Tambahkan label untuk step count dan distance
        self.stepsLabel = QLabel("Steps: ")
        self.stepsLabel.setStyleSheet("font-size: 14px;")

        self.distanceLabel = QLabel("Distance: ")
        self.distanceLabel.setStyleSheet("font-size: 14px;")        
        # Susun layout
        self.subjectSetupGrid.addWidget(self.statusLabel, 0, 0, 1, 2)
        self.subjectSetupGrid.addWidget(self.positionLabel, 1, 0, 1, 2)
        self.subjectSetupGrid.addWidget(self.stepsLabel, 2, 0, 1, 2)
        self.subjectSetupGrid.addWidget(self.distanceLabel, 3, 0, 1, 2)
    ...
    def updateGraph(self, outputDict):
        ...
        t1 = threading.Thread(target=predictModel, args=(df,))
